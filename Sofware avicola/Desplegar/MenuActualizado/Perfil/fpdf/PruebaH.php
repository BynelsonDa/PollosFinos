...
class PDF extends FPDF
{
   ...
   function Header()
   {
      include '../../../db.php';//llamamos a la conexion BD

      //$consulta1 = $bd->query("SELECT * FROM aves");//traemos datos de la empresa desde BD
      //$dato_info = $consulta1->fetch_object();
      $consulta_info = $conex->query("SELECT * FROM galpones"); // Fetch data from the 'aves' table
      $dato_info = $consulta_info->fetch_object(); // Use PDO's fetch() method with FETCH_OBJ option
      ...
      $this->Cell(70, 10, utf8_decode('Ubicacion'), 1, 0, 'C', 1);
...
$consulta_info = $conex->query("SELECT * FROM galpones");
...
$consulta_reporte_asistencia = $conex->query(" SELECT * FROM galpones ");

while ($datos_reporte = $consulta_reporte_asistencia->fetch_object()) {
   $i = $i + 1;
   /* TABLA */
   $pdf->Cell(15, 10, utf8_decode($datos_reporte->id), 1, 0, 'C', 0);
   $pdf->Cell(70, 10, utf8_decode($datos_reporte->usuario), 1, 0, 'C', 0);
   $pdf->Cell(70, 10, utf8_decode($datos_reporte->ubicacion), 1, 0, 'C', 0);
   $pdf->Cell(70, 10, utf8_decode($datos_reporte->fecha), 1, 1, 'C', 0);
