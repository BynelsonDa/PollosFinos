...
class PDF extends FPDF
{
   ...
   function Header()
   {
      ...
      $consulta_info = $bd->query("SELECT * FROM salud"); // Fetch data from the 'aves' table
      ...
      $this->Cell(15, 10, utf8_decode('id_ave'), 1, 0, 'C', 1);
      $this->Cell(40, 10, utf8_decode('nombre'), 1, 0, 'C', 1);
      $this->Cell(40, 10, utf8_decode('Medico'), 1, 0, 'C', 1);
      $this->Cell(15, 10, utf8_decode('Galpon'), 1, 0, 'C', 1);
      $this->Cell(65, 10, utf8_decode('Fecha'), 1, 0, 'C', 1);
      $this->Cell(25, 10, utf8_decode('Vacunas'), 1, 0, 'C', 1);
      $this->Cell(15, 10, utf8_decode('Coste'), 1, 0, 'C', 1);
...
$consulta_info = $bd->query("SELECT * FROM salud");
...
$consulta_reporte_asistencia = $bd->query(" SELECT * FROM salud ");

while ($datos_reporte = $consulta_reporte_asistencia->fetch(PDO::FETCH_OBJ)) {
   $i = $i + 1;
   /* TABLA */
   $pdf->Cell(15, 10, utf8_decode($datos_reporte->codigo), 1, 0, 'C', 0);
   $pdf->Cell(15, 10, utf8_decode($datos_reporte->id_ave), 1, 0, 'C', 0);
   $pdf->Cell(40, 10, utf8_decode($datos_reporte->nombre_ave), 1, 0, 'C', 0);
   $pdf->Cell(40, 10, utf8_decode($datos_reporte->Medico), 1, 0, 'C', 0);
   $pdf->Cell(15, 10, utf8_decode($datos_reporte->Galpon), 1, 0, 'C', 0);
   $pdf->Cell(65, 10, utf8_decode($datos_reporte->Fecha), 1, 0, 'C', 0);
   $pdf->Cell(25, 10, utf8_decode($datos_reporte->Vacunas), 1, 0, 'C', 0);
   $pdf->Cell(15, 10, utf8_decode($datos_reporte->Coste), 1, 0, 'C', 0);
   $pdf->Cell(45, 10, utf8_decode($datos_reporte->Alimento), 1, 1, 'C', 0);
