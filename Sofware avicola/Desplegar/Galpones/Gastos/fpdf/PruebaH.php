...
class PDF extends FPDF
{
   ...
   function Header()
   {
      ...
      $consulta_info = $bd->query("SELECT * FROM gastos"); // Fetch data from the 'aves' table
      ...
      $this->Cell(40, 10, utf8_decode('perdidas'), 1, 0, 'C', 1);
      $this->Cell(40, 10, utf8_decode('Galpon'), 1, 0, 'C', 1);
      $this->Cell(60, 10, utf8_decode('Fecha'), 1, 0, 'C', 1);
      $this->Cell(90, 10, utf8_decode('Origen'), 1, 1, 'C', 1);
   }
...
$consulta_info = $bd->query("SELECT * FROM gastos");
...
$consulta_reporte_asistencia = $bd->query(" SELECT * FROM gastos ");
...
while ($datos_reporte = $consulta_reporte_asistencia->fetch(PDO::FETCH_OBJ)) {
   ...
   $pdf->Cell(40, 10, utf8_decode($datos_reporte->Perdidas), 1, 0, 'C', 0);
   $pdf->Cell(40, 10, utf8_decode($datos_reporte->Galpon), 1, 0, 'C', 0);
   $pdf->Cell(60, 10, utf8_decode($datos_reporte->Fecha), 1, 0, 'C', 0);
   $pdf->Cell(90, 10, utf8_decode($datos_reporte->Origen), 1, 1, 'C', 0);
