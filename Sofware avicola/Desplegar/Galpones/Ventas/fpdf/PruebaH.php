...
class PDF extends FPDF
{
   ...
   function Header()
   {
      ...
      $consulta_info = $bd->query("SELECT * FROM ventas"); // Fetch data from the 'aves' table
      ...
      $this->Cell(60, 10, utf8_decode('PesoKg'), 1, 0, 'C', 1);
...
$consulta_info = $bd->query("SELECT * FROM ventas");
...
$consulta_reporte_asistencia = $bd->query(" SELECT * FROM ventas ");
...
while ($datos_reporte = $consulta_reporte_asistencia->fetch(PDO::FETCH_OBJ)) {
   ...
   $pdf->Cell(60, 10, utf8_decode($datos_reporte->Cliente), 1, 0, 'C', 0);
   $pdf->Cell(60, 10, utf8_decode($datos_reporte->PesoKg), 1, 0, 'C', 0);
